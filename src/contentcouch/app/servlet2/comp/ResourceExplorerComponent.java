...
public class ResourceExplorerComponent extends BaseComponent {
	...
	protected Response explorifyDirectory(Request req, Directory d ) {
	...
	protected Response explorifyXmlBlob(Request req, Blob b) {
		return getPageGeneratorResult(new RdfSourcePageGenerator(b, req));
	}
	
	protected Response explorifySlfBlob(Request req, Blob b) {
	...
	protected Response explorifyRef( Request req, Response subRes, Ref ref ) {
		...
		return getPageGeneratorResult(new CCouchExplorerPageGenerator(req) {
			public void writeContent(PrintWriter w) {
				w.println("<div class=\"main-content\">");
				w.println("<h3>You found a Ref!</h3>");
				w.println("<p><a href=\""+XML.xmlEscapeAttributeValue(getExternalUri("default",targetUri))+"\">"+XML.xmlEscapeText(targetUri)+"</a></p>");
				w.println("</div>");
			}
		});
	}
